...
class VerticalForm extends Controller
{
  public function index()
  {
    $user = auth()->user();
    $ciudads = Ciudad::all();
    return view('content.form-layout.form-layouts-vertical', compact('user','ciudads'));
  }
  ...
  if ($validator->fails()) {
    ...
    if ($agentes->save()) {
      ...
      return view('content.form-layout.form-layouts-vertical', compact('sectors'));
    }
  }
  ...
  public function agentesPorPropiedad()
{
  ...
  $agentes = Agentes::where('inmobiliaria_id', $inmobiliaria->id)->get();
    ...
    return view('content.form-layout.form-layouts-vertical', compact('agentes','ciudads','user'));
}
}
